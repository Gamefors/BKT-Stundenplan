(function() {
	...
	function ScheduleTemplate( element ) {
		...
		var lessonList = document.getElementsByClassName('cd-schedule__group')
		var mondayUl = lessonList[0].getElementsByTagName('ul')[0]
		var tuesdayUl = lessonList[1].getElementsByTagName('ul')[0]
		var wednesdayUl = lessonList[2].getElementsByTagName('ul')[0]
		var thursdayUl = lessonList[3].getElementsByTagName('ul')[0]
		var fridayUl = lessonList[4].getElementsByTagName('ul')[0]
		...
		function createEvent(start, end, name, color){
			...
			a.setAttribute("data-end", end)
			a.setAttribute("data-content", name)
			a.setAttribute("data-event", color)
			a.appendChild(em)
		...
		function populateDay(day, data){
			...
			day.appendChild(createEvent("07:30", "09:00", data[0].Subject, "event-1"))

			day.appendChild(createEvent("09:00", "09:15", "Pause", "event-2"))

			day.appendChild(createEvent("09:15", "10:45", data[3].Subject, "event-1"))

			day.appendChild(createEvent("10:45", "11:00", "Pause", "event-2"))

			day.appendChild(createEvent("11:00", "12:30", data[5].Subject, "event-1"))

			day.appendChild(createEvent("12:30", "12:45", "Pause", "event-2"))
			
			day.appendChild(createEvent("12:45", "14:15", data[7].Subject, "event-1"))

			day.appendChild(createEvent("14:15", "14:30", "Pause", "event-2"))

			day.appendChild(createEvent("14:30", "16:00", data[9].Subject, "event-4"))
		}
		...
		Http.onreadystatechange = (e) => {
			if (Http.readyState === XMLHttpRequest.DONE) {
				if (Http.status === 200) {
					...
					populateDay(mondayUl, dbLessonData.Monday)
					populateDay(tuesdayUl, dbLessonData.Tuesday)
					populateDay(wednesdayUl, dbLessonData.Wednesday)
					populateDay(thursdayUl, dbLessonData.Thursday)
					populateDay(fridayUl, dbLessonData.Friday)
					this.placeEvents()
					this.initEvents()
				}
			}
		...
		this.timelineUnitDuration = this.timelineUnitDuration/3
	...
	ScheduleTemplate.prototype.placeEvents = function() {
		...
		for(var i = 0; i < this.singleEvents.length; i++) {
			...
			var topDuration = start - self.timelineStart

			if(topDuration >= 90){
				topDuration = topDuration - 70
			}
			if(topDuration >= 105){
				topDuration = topDuration - 70
			}
			...
			var eventTop = 50*(topDuration)/self.timelineUnitDuration,
