    ...
    return (
      <View style={{ margin: 15 }}>
        <Text style={{ fontSize: 25, color: color1 }}>
          ...
          <Text style={{ color: "#59886b" }}>{day1.Room}</Text>
        </Text>
        ...
        <Text style={{ fontSize: 25, color: color1 }}>
          ...
          <Text style={{ color: "#726a95" }}>{" | "}</Text>
          <Text style={{ color: "#59886b" }}>{day2.Room}</Text>
        </Text>
      </View>
    );
  } else {
...
function lessons({ route }) {
  ...
  return (
    <View style={{ flex: 1, alignItems: "center", backgroundColor: "#2C2F33", width: "100%", height: "100%" }}>
...
function timetable(weeks) {
  ...
  return (
    <NavigationContainer>
      ...
      <Tab.Navigator
        initialRouteName={daysForIndex[currDate.getDay() - 1 + afternoon]}
        tabBarOptions={{
          scrollEnabled: true,
...
function loadingScreen() {
  ...
  if (currView.props.children.length > 2) {
    let fetching = setInterval(function () {
      getRequest(FETCH_SERVER_ADRESS)
        .then((data) => {
